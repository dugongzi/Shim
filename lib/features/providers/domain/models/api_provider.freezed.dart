...
mixin _$ApiProvider {
 ...
 String get upstreamProtocol;/// Provider-level weight 1..10 (default 5). Used in auto-switch ranking:
/// score = providerWeight * modelWeight * (1 / latencyMs).
/// Higher weight = preferred candidate when current provider fails.
 int get providerWeight;/// Model-level weight 1..10 (default 5). Same ranking formula.
 int get modelWeight;
...
@override
String toString() {
  return 'ApiProvider(id: $id, name: $name, baseUrl: $baseUrl, apiKey: $apiKey, models: $models, selectedModel: $selectedModel, upstreamProtocol: $upstreamProtocol, providerWeight: $providerWeight, modelWeight: $modelWeight)';
}
...
class _ApiProvider extends ApiProvider {
  const _ApiProvider({required this.id, required this.name, required this.baseUrl, required this.apiKey, required final  List<String> models, required this.selectedModel, required this.upstreamProtocol, required this.providerWeight, required this.modelWeight}): _models = models,super._();
...
/// score = providerWeight * modelWeight * (1 / latencyMs).
/// Higher weight = preferred candidate when current provider fails.
@override final  int providerWeight;
/// Model-level weight 1..10 (default 5). Same ranking formula.
@override final  int modelWeight;
